...
#knot positions
optim.knots.10knot=quantile(log(t[d==1]),c(seq(0,1-1/11,1/11),1))
optim.myknots=optim.knots.10knot #I rename here just to save myself some copy and pasting in the loglik function below. 

maxknot=12 #total number of knots (including boundary knots)
...
loglik=function(params){
  ...
  loglog.survfunc=gamma0+gamma1*log(t)+
    ...
    gamma11*sapply(t,FUN=function(t)v(t,optim.myknots[1],optim.myknots[maxknot],optim.myknots[11]))
  ...
  hazfunc=(gamma1*(1/t)+
             ...
             gamma11*sapply(t,FUN=function(t)v.deriv(t,optim.myknots[1],optim.myknots[maxknot],optim.myknots[11])))*
...
optim.fit.10knot=optim(flexmod.10knot$coefficients,loglik,method="BFGS",hessian=F)

myhess.10knot=fdHess(flexmod.10knot$coefficients,loglik)

varcov.myhess.10knot=solve(myhess.10knot$Hessian)
