...
#knot positions
optim.knots.2knot=quantile(log(t[d==1]),c(seq(0,1-1/3,1/3),1))
optim.myknots=optim.knots.2knot #I rename here just to save myself some copy and pasting in the loglik function below. 

maxknot=4 #total number of knots (including boundary knots)
...
optim.fit.2knot=optim(flexmod.2knot$coefficients,loglik,method="BFGS",hessian=F)

myhess.2knot=fdHess(flexmod.2knot$coefficients,loglik)

varcov.myhess.2knot=solve(myhess.2knot$Hessian)
