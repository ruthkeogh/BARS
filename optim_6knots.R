...
#knot positions
optim.knots.6knot=quantile(log(t[d==1]),c(seq(0,1-1/7,1/7),1))
optim.myknots=optim.knots.6knot #I rename here just to save myself some copy and pasting in the loglik function below. 

maxknot=8 #total number of knots (including boundary knots)
...
optim.fit.6knot=optim(flexmod.6knot$coefficients,loglik,method="BFGS",hessian=F)

myhess.6knot=fdHess(flexmod.6knot$coefficients,loglik)

varcov.myhess.6knot=solve(myhess.6knot$Hessian)
