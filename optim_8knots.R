...
#knot positions
optim.knots.8knot=quantile(log(t[d==1]),c(seq(0,1-1/9,1/9),1))
optim.myknots=optim.knots.8knot #I rename here just to save myself some copy and pasting in the loglik function below. 

maxknot=10 #total number of knots (including boundary knots)
...
optim.fit.8knot=optim(flexmod.8knot$coefficients,loglik,method="BFGS",hessian=F)

myhess.8knot=fdHess(flexmod.8knot$coefficients,loglik)

varcov.myhess.8knot=solve(myhess.8knot$Hessian)
